...
; Read in the 2d spectrum
 ...
 ; fname1 = 'slit' $ + '.' + strcompress(results[slitcount].slitname, /remove_all) $
 ;          + 'B.fits'
;abs comment for changing slit file names
...
; displayed in the "Fit for z" widget.
  ...
  path = topdir + '/' + maskname + '/spec1dRedux/zresult'
  ;print, "YOUR PATH IS: ", path ;abs comment
; path modified by abs 6/2/17, trying to doctor my results file
; organization into DEEP schemes abs comment
  ...
  IF ct NE 1 THEN BEGIN
    ...
    ;print, "you just picked your fname from the prompt>>>>>>>>>>>>>>>>>>>";ABS debug comment
  endif else $
    fname = filelist[0]
    ;print, "The fname was picked for you from the first element in the filelist.>>>>>>>>>>>>>>>>>>>";ABS debug comment
    ;print,"which is: ",fname ; ABS debug comment
  ...
  filelist=findfile(topdir +'/' + maskname + '/*/obj_info*.fits', count=nobjfile)
   ;abs comment finding the obj_info files
  ;filelist=findfile(topdir +'/' + maskname + '/obj_info*.fits', count=nobjfile)
  ...
  ;   print, zbest.slitname[i]
  ;   IF(STRMID(zbest.objname[i],0,1) EQ 's') THEN BEGIN
  ;      print, 'new serendip object assignment:',zbest.slitname[i] + '.' + zbest.objname[i]
  ;   ENDIF
  ;ENDFOR

  ; Remove serendips from the list
  ;--------------------------------
;ABS - commented out serendip block to try to get zspec to read
;      serendip objects.
 ; serendip_count = 0
 ; FOR i=0, nslit-1 DO BEGIN
 ...
 ; print, 'No of serendips: ', serendip_count
  ...
  ;  IF(STRMID(objlist[i],0,1) NE 's') THEN BEGIN
       ...
       serendip = serendip + 1
  ;  ENDIF
  ENDFOR ;ABS commented out serendip filtering IF statement, trying to get 
        ; serendip objs into the zspec catalog creation 07/11/17
  ;print, "objlist:",objlist, "objnames:",objnames ;abs comment
  ;slitnames = slnames
